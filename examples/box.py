...
#
# Description: XYLA colors on width and height sides
# Uses: Primary #0000ff00, Secondary #ff000000
...
def update(strip, table_values):
    global transition, time_start, target_primary, target_secondary, did_show
    if time_start == 0:
        time_start = timer()
        transition = 0
    ...
    if transition < 1.0:
        ...
        for i in range(int(height), int(height+width)):
            strip.setPixelColor(i, colorBlend(strip.getPixelColor(i),target_primary,easeOut(transition)))
        for i in range(int(height+width), int(height+width+height)):
            strip.setPixelColor(i, colorBlend(strip.getPixelColor(i),target_secondary,easeOut(transition)))
        for i in range(int(height+width+height), strip.numPixels()):
            strip.setPixelColor(i, colorBlend(strip.getPixelColor(i),target_primary,easeOut(transition)))
        ...
        for i in range(int(height), int(height+width)):
            strip.setPixelColor(i, target_primary)
        for i in range(int(height+width), int(height+width+height)):
            strip.setPixelColor(i, target_secondary)
        for i in range(int(height+width+height), strip.numPixels()):
            strip.setPixelColor(i, target_primary)
